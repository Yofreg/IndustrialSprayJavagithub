 ...
 * @desc socket本地通讯 第五个喷头
 */
public class LocalControler5 implements Runnable {
    ...
    private void pwsend(byte[] f) throws IOException {
        ...
        if (timer == null) {
            ...
            timer.schedule(new TimerTask() {
                @Override
                public void run() {
                    SocketBeanFS.ebeanFS("服务器无返回", 5, 0);

                }
            }, 2000);
        }
        SocketIsAndOs.OsWrite(pw, f, 5);
    }
    ...
    public void send(byte type, byte[] datas){
        ...
        if (mSocket != null && mSocket.isConnected()) {
            try {
                ...
                SocketBeanFS.ebeanFS("通讯连接不上", 5, 1);
            }
        } else {
            SocketBeanFS.ebeanFS("通讯连接不上", 5, 2);
        }
    }

    /**
     * 发送
     * @param bean PMJstorageBean类型数据
     */
    /*public void send2(PMJstorageBean bean){
            ...
            try {
                ...
                *//*
                ...
                b[0] = (byte) (jiaoyan&0xff);*//*
                    ...
                    //正式用
                    byte[] a = PMJBasicCons.TCP_TBT.getBytes("utf-8");
                    byte[] payload = new byte[1 + 4 + bean.getsJM().length];
                    payload[0] = bean.getlXM()[0];
                    long len_tmp = bean.getsJM().length;
                    payload[1] = (byte)(len_tmp & 0xff); len_tmp=len_tmp >> 8;
                    payload[2] = (byte)(len_tmp & 0xff); len_tmp=len_tmp >> 8;
                    payload[3] = (byte)(len_tmp & 0xff); len_tmp=len_tmp >> 8;
                    payload[4] = (byte)(len_tmp & 0xff);
                    for(int i = 0; i < bean.getsJM().length; i++) {
                        payload[5 + i] = bean.getsJM()[i];
                    }
                    long jiaoyan = CRC32Utils.byteCRC32(payload);
                    byte[] b = new byte[4];
                    b[0] = (byte) (jiaoyan & 0xff); jiaoyan = jiaoyan >> 8;
    ...
    public void sendQDDY(byte[] datas){
        ...
        if (mSocket != null && mSocket.isConnected()) {
            try {
                ...
                SocketBeanFS.ebeanFS("通讯连接不上", 5, 3);
            }
        } else {
            SocketBeanFS.ebeanFS("通讯连接不上", 5, 4);
        }
    }
    ...
    public void sendSZCS(byte[] datas) {
        if (mSocket != null && mSocket.isConnected()) {
            try {
                ...
                SocketBeanFS.ebeanFS("通讯连接不上", 5, 17);
            }
        } else {
            SocketBeanFS.ebeanFS("通讯连接不上", 5, 18);
        }
    }
            ...
            try {
                ...
                if (timer2 == null) {
                    ...
                    timer2.schedule(new TimerTask() {
                        @Override
                        public void run() {
                            SocketBeanFS.ebeanFS("服务器无返回", 5, 1);
                ...
                SocketBeanFS.ebeanFS("通讯连接不上", 5, 5);
            }
        } else {
            SocketBeanFS.ebeanFS("通讯连接不上", 5, 6);
        }
    }
    ...
    public void sendKZDY(byte[] datas){
        ...
        if (mSocket != null && mSocket.isConnected()) {
            try {
                ...
                SocketBeanFS.ebeanFS("通讯连接不上", 5, 7);
            }
        } else {
            SocketBeanFS.ebeanFS("通讯连接不上", 5, 8);
        }
    }
    ...
    public void sendSCXX(byte[] datas){
        ...
        if (mSocket != null && mSocket.isConnected()) {
            try {
                ...
                SocketBeanFS.ebeanFS("通讯连接不上", 5, 9);
            }
        } else {
            SocketBeanFS.ebeanFS("通讯连接不上", 5, 10);
        }
    }
    ...
    public void sendCXXX(byte[] datas){
        ...
        if (mSocket != null && mSocket.isConnected()) {
            try {
                ...
                SocketBeanFS.ebeanFS("通讯连接不上", 5, 11);
            }
        } else {
            SocketBeanFS.ebeanFS("通讯连接不上", 5, 12);
        }
    }
    ...
    public void sendSJCX(byte[] datas){
        ...
        if (mSocket != null && mSocket.isConnected()) {
            try {
                ...
                SocketBeanFS.ebeanFS("通讯连接不上", 5, 13);
            }
        } else {
            SocketBeanFS.ebeanFS("通讯连接不上", 5, 14);
        }
    }
    ...
    private void initSocket() {
        try {
            ...
            mSocket = new Socket(PMJBasicCons.DATA_IP5, PMJBasicCons.DATA_PORT);
            ...
            if (mSocket.isConnected()) {
                ...
                SocketBeanFS.ebeanFS("通讯连接不上", 5, 15);
            }

        } catch (IOException e) {
            SocketBeanFS.ebeanFS("通讯连接不上", 5, 16);
        }
        //循环获取设备传递的数据
        while (isRun){
            try {
                if (is != null) {
                    ...
                    if (rcvLen != -1) {
                        ...
                        String a = SocketIsAndOs.isRead(rcvLen, buff, 5, timer, timer2);
                        if (a.contains("1")) {
                            if (timer != null) {
                                timer.cancel();
                                timer = null;
