 ...
 * @desc socket本地通讯 第四个喷头
 */
public class LocalControler4 implements Runnable {
    ...
    private void pwsend(byte[] f) throws IOException {
        ...
        if (timer == null) {
            ...
            timer.schedule(new TimerTask() {
                @Override
                public void run() {
                    SocketBeanFS.ebeanFS("服务器无返回", 4, 0);

                }
            }, 2000);
        }
        SocketIsAndOs.OsWrite(pw, f, 4);
    }
    ...
    public void send(byte type, byte[] datas){
        ...
        if (mSocket != null && mSocket.isConnected()) {
            try {
                ...
                SocketBeanFS.ebeanFS("通讯连接不上", 4, 1);
            }
        } else {
            SocketBeanFS.ebeanFS("通讯连接不上", 4, 2);
        }
    }
    ...
    public void sendQDDY(byte[] datas){
        ...
        if (mSocket != null && mSocket.isConnected()) {
            try {
                ...
                SocketBeanFS.ebeanFS("通讯连接不上", 4, 3);
            }
        } else {
            SocketBeanFS.ebeanFS("通讯连接不上", 4, 4);
        }
    }
    ...
    public void sendSZCS(byte[] datas) {
        if (mSocket != null && mSocket.isConnected()) {
            try {
                ...
                SocketBeanFS.ebeanFS("通讯连接不上", 4, 17);
            }
        } else {
            SocketBeanFS.ebeanFS("通讯连接不上", 4, 18);
        }
    }
            ...
            try {
                ...
                if (timer2 == null) {
                    ...
                    timer2.schedule(new TimerTask() {
                        @Override
                        public void run() {
                            SocketBeanFS.ebeanFS("服务器无返回", 4, 1);
                ...
                SocketBeanFS.ebeanFS("通讯连接不上", 4, 5);
            }
        } else {
            SocketBeanFS.ebeanFS("通讯连接不上", 4, 6);
        }
    }
    ...
    public void sendKZDY(byte[] datas){
        ...
        if (mSocket != null && mSocket.isConnected()) {
            try {
                ...
                SocketBeanFS.ebeanFS("通讯连接不上", 4, 7);
            }
        } else {
            SocketBeanFS.ebeanFS("通讯连接不上", 4, 8);
        }
    }
    ...
    public void sendSCXX(byte[] datas){
        ...
        if (mSocket != null && mSocket.isConnected()) {
            try {
                ...
                SocketBeanFS.ebeanFS("通讯连接不上", 4, 9);
            }
        } else {
            SocketBeanFS.ebeanFS("通讯连接不上", 4, 10);
        }
    }
    ...
    public void sendCXXX(byte[] datas){
        ...
        if (mSocket != null && mSocket.isConnected()) {
            try {
                ...
                SocketBeanFS.ebeanFS("通讯连接不上", 4, 11);
            }
        } else {
            SocketBeanFS.ebeanFS("通讯连接不上", 4, 12);
        }
    }
    ...
    public void sendSJCX(byte[] datas){
        ...
        if (mSocket != null && mSocket.isConnected()) {
            try {
                ...
                SocketBeanFS.ebeanFS("通讯连接不上", 4, 13);
            }
        } else {
            SocketBeanFS.ebeanFS("通讯连接不上", 4, 14);
        }
    }
    ...
    private void initSocket() {
        try {
            ...
            mSocket = new Socket(PMJBasicCons.DATA_IP4, PMJBasicCons.DATA_PORT);
            ...
            if (mSocket.isConnected()) {
                ...
                SocketBeanFS.ebeanFS("通讯连接不上", 4, 15);
            }

        } catch (IOException e) {
            SocketBeanFS.ebeanFS("通讯连接不上", 4, 16);
        }
        //循环获取设备传递的数据
        while (isRun){
            try {
                if (is != null) {
                    ...
                    if (rcvLen != -1) {
                        ...
                        String a = SocketIsAndOs.isRead(rcvLen, buff, 4, timer, timer2);
                        if (a.contains("1")) {
                            if (timer != null) {
                                timer.cancel();
                                timer = null;
