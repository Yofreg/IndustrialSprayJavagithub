 ...
 * @desc socket本地通讯 第二个喷头
 */
public class LocalControler2 implements Runnable {
    ...
    private void pwsend(byte[] f) throws IOException {
        ...
        if (timer == null) {
            ...
            timer.schedule(new TimerTask() {
                @Override
                public void run() {
                    SocketBeanFS.ebeanFS("服务器无返回", 2, 0);

                }
            }, 2000);
        }
        SocketIsAndOs.OsWrite(pw, f, 2);
    }
    ...
    public void send(byte type, byte[] datas){
        ...
        if (mSocket != null && mSocket.isConnected()) {
            try {
                ...
                SocketBeanFS.ebeanFS("通讯连接不上", 2, 1);
            }
        } else {
            SocketBeanFS.ebeanFS("通讯连接不上", 2, 2);
        }
    }
    ...
    public void sendQDDY(byte[] datas){
        ...
        if (mSocket != null && mSocket.isConnected()) {
            try {
                ...
                SocketBeanFS.ebeanFS("通讯连接不上", 2, 3);
            }
        } else {
            SocketBeanFS.ebeanFS("通讯连接不上", 2, 4);
        }
    }
    ...
    public void sendSZCS(byte[] datas) {
        if (mSocket != null && mSocket.isConnected()) {
            try {
                ...
                SocketBeanFS.ebeanFS("通讯连接不上", 2, 17);
            }
        } else {
            SocketBeanFS.ebeanFS("通讯连接不上", 2, 18);
        }
    }
            ...
            try {
                ...
                if (timer2 == null) {
                    ...
                    timer2.schedule(new TimerTask() {
                        @Override
                        public void run() {
                            SocketBeanFS.ebeanFS("服务器无返回", 2, 1);
                ...
                SocketBeanFS.ebeanFS("通讯连接不上", 2, 5);
            }
        } else {
            SocketBeanFS.ebeanFS("通讯连接不上", 2, 6);
        }
    }
    ...
    public void sendKZDY(byte[] datas){
        ...
        if (mSocket != null && mSocket.isConnected()) {
            try {
                ...
                SocketBeanFS.ebeanFS("通讯连接不上", 2, 7);
            }
        } else {
            SocketBeanFS.ebeanFS("通讯连接不上", 2, 8);
        }
    }
    ...
    public void sendSCXX(byte[] datas){
        ...
        if (mSocket != null && mSocket.isConnected()) {
            try {
                ...
                SocketBeanFS.ebeanFS("通讯连接不上", 2, 9);
            }
        } else {
            SocketBeanFS.ebeanFS("通讯连接不上", 2, 10);
        }
    }
    ...
    public void sendCXXX(byte[] datas){
        ...
        if (mSocket != null && mSocket.isConnected()) {
            try {
                ...
                SocketBeanFS.ebeanFS("通讯连接不上", 2, 11);
            }
        } else {
            SocketBeanFS.ebeanFS("通讯连接不上", 2, 12);
        }
    }
    ...
    public void sendSJCX(byte[] datas){
        ...
        if (mSocket != null && mSocket.isConnected()) {
            try {
                ...
                SocketBeanFS.ebeanFS("通讯连接不上", 2, 13);
            }
        } else {
            SocketBeanFS.ebeanFS("通讯连接不上", 2, 14);
        }
    }
    ...
    private void initSocket() {
        try {
            ...
            mSocket = new Socket(PMJBasicCons.DATA_IP2, PMJBasicCons.DATA_PORT);
            ...
            if (mSocket.isConnected()) {
                ...
                SocketBeanFS.ebeanFS("通讯连接不上", 2, 15);
            }

        } catch (IOException e) {
            SocketBeanFS.ebeanFS("通讯连接不上", 2, 16);
        }
        //循环获取设备传递的数据
        while (isRun){
            try {
                if (is != null) {
                    ...
                    if (rcvLen != -1) {
                        ...
                        String a = SocketIsAndOs.isRead(rcvLen, buff, 2, timer, timer2);
                        if (a.contains("1")) {
                            if (timer != null) {
                                timer.cancel();
                                timer = null;
