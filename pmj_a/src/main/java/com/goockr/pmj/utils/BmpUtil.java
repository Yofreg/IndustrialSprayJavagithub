...
public class BmpUtil {
    ...
    public static byte[] changeSingleBytes(Bitmap bmp) {
        int[] binarys = gray2Binary(bmp);
        return compressMonoBitmap_ps(bmp, binarys);
    }
    ...
    private static byte[] addBMPImageInfosHeader(int w, int h) {
        ...
        int widthBytes = (w + 31) / 32 * 4 * h;

        byte[] bb = LongIntToBytesUtils.intToBytesXDTY(widthBytes, 4);
        buffer[20] = bb[0];
        buffer[21] = bb[1];
        buffer[22] = bb[2];
        buffer[23] = bb[3];
